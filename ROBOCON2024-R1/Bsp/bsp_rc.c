...
void rc_callback_handler(rc_info_t *rc, uint8_t *buff)
{
  ...
  rc->ch[4] -= 1024;
  rc->ch[5] = (buff[6] | buff[7] << 8);
  rc->ch[6] = (buff[8] | buff[9] << 8);


  rc->sw[0] = ((buff[5] >> 4) & 0x000C) >> 2;
  rc->sw[1] = (buff[5] >> 4) & 0x0003;
  rc->mKey = (buff[14] | buff[15]<<8);
  // rc->key[0] = (buff[14] >> 7) & 0x0001;
  // rc->key[1] = (buff[14] >> 6) & 0x0001;
  // rc->key[2] = (buff[14] >> 5) & 0x0001;
  // rc->key[3] = (buff[14] >> 4) & 0x0001;
  // rc->key[4] = (buff[14] >> 3) & 0x0001;
  // rc->key[5] = (buff[14] >> 2) & 0x0001;
  // rc->key[6] = (buff[14] >> 1) & 0x0001;
  // rc->key[7] = buff[14] & 0x0001;
  // rc->key[8] = buff[15] & 0x0001;
...
/**
  * @brief      callback this function when uart interrupt
	* @note				挂到stm32f4xx_it.h的串口3中断函数中
  * @param[in]  huart: uart IRQHandler id
  * @retval  
  */
