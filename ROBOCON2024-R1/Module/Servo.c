...
//  * @brief  舵机初始化
//  */
//void servo_init(servo_t *servo,TIM_HandleTypeDef htim,uint8_t tim_channel,servo_direction direction)
//{
//	servo->htim=htim;
...
///**
//  * @brief  用pwm直接控制舵机
//	* @note angle是舵机转动的绝对角度，具体多少度夹爪闭合与机械装配有关
//  */
...
/**
  * @brief  舵机初始化
  */
void servo_init_lsc(servo_t *servo,uint8_t id,servo_direction direction,uint16_t time)
{
	servo->id = id;
	servo->direction = direction;
	servo->time = time;
...
/**
  ...
  * @param servo 舵机的结构体
  */
void move_servo_lsc(servo_t *servo,uint16_t position)
{
