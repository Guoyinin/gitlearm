...
/**     V1.0.2  2024-05-23          bear_king         将硬件层也改为面向对象的操作                                                          **/
/******************************************************************************/ 
...
/*写寄存器*/
static uint8_t NRF24L01WriteRegister(nrf24l01_t *nrf,uint8_t reg,uint8_t value);
/*读取寄存器值*/
static uint8_t NRF24L01ReadRegigster(nrf24l01_t *nrf,uint8_t reg);
/*在指定位置读出指定长度的数据*/
static uint8_t NRF24L01ReadBuffer(nrf24l01_t *nrf,uint8_t reg,uint8_t *pBuf,uint8_t len);
/*在指定位置写指定长度的数据*/
static uint8_t NRF24L01WriteBuffer(nrf24l01_t *nrf,uint8_t reg, uint8_t *pBuf, uint8_t len);
/*检测24L01是否存在,返回值:0，成功;1，失败*/
static uint8_t NRF24L01Check(nrf24l01_t *nrf);
/*设置nRF24L01的模式*/
 void SetNRF24L01Mode(nrf24l01_t *nrf,NRF24L01ModeType mode);
/*缺省片选处理函数*/
static void NRF24L01CSDefault(nrf24l01_t* nrf, NRF24L01CSType cs);
				   
/*启动NRF24L01发送一次数据包*/
/*参数：txbuf:待发送数据首地址*/
/*返回值：发送完成状况*/
uint8_t NRF24L01TransmitPacket(nrf24l01_t *nrf,uint8_t *txbuf)
{
  uint8_t status;

  SetNRF24L01Mode(nrf,NRF24L01TxMode);
	
  nrf->ChipEnable(nrf, NRF24L01CE_Disable);
  NRF24L01WriteBuffer(nrf,WR_TX_PLOAD,txbuf,TX_PLOAD_WIDTH);//写数据到TX BUF 32个字节
  nrf->ChipEnable(nrf, NRF24L01CE_Enable);//启动发送	   
  
  while(nrf->GetIRQ(nrf)!=0);              //等待发送完成
...
/*参数：txbuf:待发送数据首地址*/
/*返回值:0，接收完成；其他，错误代码*/
uint8_t NRF24L01ReceivePacket(nrf24l01_t *nrf,uint8_t *rxbuf)
{ 
...
/*nRF24L01对象初始化函数*/
NRF24L01ErrorType NRF24L01Initialization(nrf24l01_t *nrf,	                    //nRF24L01对象
                                         ...
                                         bsp_nrf24l01_t bsp_nrf24l01)
{
  int retry=0;
	nrf->bsp_nrf24l01 = bsp_nrf24l01;
...
void SetNRF24L01Mode(nrf24l01_t *nrf, NRF24L01ModeType mode)
{
  ...
  nrf->ChipEnable(nrf, NRF24L01CE_Enable); //CE为高。设置RX时，进入接收模式；设置为TX时,10us后启动发送
}
...
static uint8_t NRF24L01WriteRegister(nrf24l01_t *nrf,uint8_t reg,uint8_t value)
{
  ...
  nrf->ChipSelect(nrf, NRF24L01CS_Enable);             //使能SPI传输
  status =nrf->ReadWriteByte(nrf, reg);   //发送寄存器号 
  nrf->ReadWriteByte(nrf, value);         //写入寄存器的值
  nrf->ChipSelect(nrf, NRF24L01CS_Disable);            //禁止SPI传输
...
static uint8_t NRF24L01ReadRegigster(nrf24l01_t *nrf,uint8_t reg)
{
  ...
  nrf->ReadWriteByte(nrf, reg);           //发送寄存器号
  reg_val=nrf->ReadWriteByte(nrf, 0XFF);  //读取寄存器内容
  nrf->ChipSelect(nrf, NRF24L01CS_Disable);            //禁止SPI传输
...
static uint8_t NRF24L01ReadBuffer(nrf24l01_t *nrf,uint8_t reg,uint8_t *pBuf,uint8_t len)
{
  ...
  status=nrf->ReadWriteByte(nrf, reg);    //发送寄存器值(位置),并读取状态值 
        
  for(int i=0;i<len;i++)
  {
    pBuf[i]=nrf->ReadWriteByte(nrf, 0XFF);//读出数据
  }
        
  nrf->ChipSelect(nrf, NRF24L01CS_Disable);            //关闭SPI传输
...
static uint8_t NRF24L01WriteBuffer(nrf24l01_t *nrf,uint8_t reg, uint8_t *pBuf, uint8_t len)
{
  ...
  for(int i=0; i<len; i++)
  {
    nrf->ReadWriteByte(nrf, pBuf[i]);     //写入数据	 
  }
...
/* 基于HAL库的SPI读写字节函数 */
uint8_t NRF24L01ReadWrite(nrf24l01_t* nrf, uint8_t txData)
{
  uint8_t rxData=0;

  HAL_SPI_TransmitReceive(nrf->bsp_nrf24l01.bsp_nrf24l01_spi,&txData,&rxData,1,1000);

  return rxData;
}

/*实现片选*/
void NRF24L01ChipSelectf(nrf24l01_t* nrf, NRF24L01CSType cs)
{
	if(NRF24L01CS_Enable == cs)
	{
	  HAL_GPIO_WritePin(nrf->bsp_nrf24l01.bsp_nrf24l01_chip_select_port, nrf->bsp_nrf24l01.bsp_nrf24l01_chip_select_pin, GPIO_PIN_RESET);
	}
	else
	{
	  HAL_GPIO_WritePin(nrf->bsp_nrf24l01.bsp_nrf24l01_chip_select_port, nrf->bsp_nrf24l01.bsp_nrf24l01_chip_select_pin, GPIO_PIN_SET);
	}
}

/*实现使能*/
void NRF24L01ChipEnablef(nrf24l01_t* nrf, NRF24L01CEType en)
{
	if(NRF24L01CE_Enable==en)
	{
		HAL_GPIO_WritePin(nrf->bsp_nrf24l01.bsp_nrf24l01_chip_enable_port, nrf->bsp_nrf24l01.bsp_nrf24l01_chip_enable_pin, GPIO_PIN_RESET);
	}
	else
	{
		HAL_GPIO_WritePin(nrf->bsp_nrf24l01.bsp_nrf24l01_chip_enable_port, nrf->bsp_nrf24l01.bsp_nrf24l01_chip_enable_pin, GPIO_PIN_SET);
	}
}
...
{
	return HAL_GPIO_ReadPin(nrf->bsp_nrf24l01.bsp_nrf24l01_chip_iqr_port, nrf->bsp_nrf24l01.bsp_nrf24l01_chip_iqr_pin);
}
